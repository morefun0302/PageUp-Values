...
//Rare exceptions to indicate a bug in the _pageupvalues code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_pageupvalues = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_pageupvalues]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.pageupvalues.module"
